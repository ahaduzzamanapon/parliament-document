...
@section('main-content')
    <div class="w-full px-5 lg:ps-8  bg-[#F2F8FF]">
        <p class="font-solaimans text-15 text-black leading-4 py-4">{{ __('messages.Home') }} / {{ __('messages.share') }}
            {{ __('messages.file_manager') }}</p>
    </div>
    <section class="px-10  py-6 h-full">
        <div class=" bg-white rounded-md px-4 py-6 shadow">
            <div class="lg:flex justify-between items-center bg-white pb-3">
                <h3 class="font-solaimans text-20 leading-5 w-full lg:w-[40%] xl:w-[25%]">{{ __('messages.share') }}
                    {{ __('messages.file_manager') }}</h3>
                        ...
                        <tr>
                            <th>
                                <h2 class="text-[#007A43] text-18 font-solaimans font-semibold">
                                    {{ __('messages.Shared by Name') }}</h2>
                            </th>

                            <th class="ps-9">
                                <h2 class="text-[#007A43] text-18 font-solaimans font-semibold ">{{ __('messages.date') }}
                                    {{ __('messages.and') }} {{ __('messages.time') }} </h2>
                            </th>
                            <th>
                                <h2 class="text-[#007A43] text-18 font-solaimans font-semibold text-center">
                                    {{ __('messages.type') }}</h2>
                            </th>
                            <th>
                              <h2 class="text-[#007A43] text-18 font-solaimans font-semibold text-center">
                        ...
                        @foreach ($share_list as $share)
                            <tr>
                                <th class="text-[#082303] font-solaimans text-16 flex items-center gap-2">
                                    <figure>
                                        <img class="w-9 h-8" src="{{ asset('uploads/prp-users/' . $share->photo) }}"
                                            alt="">
                                    </figure>
                                    <span class="text-[#082303] font-solaimans text-16 leading-4">
                                        {{ __('messages.Current_Language') == 'en' ? $share->nameEn : $share->nameBn }}
                                    </span>
                                </th>
                                ...
                                <td>
                                    <div class="flex items-center gap-4">
                                        <span class="text-[#5C5F62] text-15 font-solaimans">
                                            {{ date('Y-m-d', strtotime($share->date)) }}
                                        </span>
                                        <span class="text-[#5C5F62] text-15 font-solaimans">
                                            {{ date('h:i A', strtotime($share->time)) }}
                                        </span>
                                    </div>
                                </td>
                                <td>
                                    <span class="text-[#082303] font-solaimans text-16 leading-4">
                                        {{ $share->document_type == 'file' ? __('messages.file') : __('messages.folder') }}
                                    </span>
                                </td>
                                <td>
                                  @if ($share->document_type == 'folder')
                                  <?php $data= DB::table('categories')->where('id', $share->document_id)->pluck('name') ?>
                                  @else
                                  <?php $data= DB::table('documents')->where('id', $share->document_id)->pluck('title') ?>
                                  @endif
                                  <?=
                                  (isset($data[0])) ? $data[0] : 'Data Not Found'
                                  ?>
                              </td>
                                <td>
                                    <div class="flex items-center justify-center  gap-5">
                                        ...
                                        <button
                                            onclick="gotofileshare({{ $share->document_id }},'{{ $share->document_type }}')"
                                            class="border border-[#00A6B0] flex items-center gap-1 px-3 py-1 rounded">
                                            @if ($share->document_type == 'folder')
                                                <span>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="15"
                                                        viewBox="0 0 16 15" fill="none">
                                                        <path
                                                            d="M2.03809 13.125H12.3506L12.8006 12.7781L15.2662 6.21563L14.8162 5.625H13.7568V3.28125L13.2881 2.8125H7.85996L7.05371 2.01563L6.72559 1.875H2.03809L1.56934 2.34375V12.6562L2.03809 13.125ZM2.50684 2.8125H6.52871L7.33496 3.60937L7.66309 3.75H12.8193V5.625H8.60059L8.27246 5.76562L7.46621 6.5625H3.91309L3.47246 6.88125L2.53496 9.76875L2.50684 2.8125ZM12.0037 12.1875H2.68496L4.25059 7.5H7.66309L7.99121 7.35938L8.79746 6.5625H14.2256L12.0037 12.1875Z"
                                                            fill="#00A6B0" />
                                                    </svg>
                                            ...
                                            <a class="text-[#00A6B0] font-solaimans text-16">
                                                @if ($share->document_type == 'folder')
                                                    {{ __('messages.Borowse Folder') }}
                                                @else
                                                    {{ __('messages.view file') }}
                                                @endif
                                            </a>
                                        </button>
                                ...
                                <td>
                                    ...
                                    @else
                                        <div class="flex items-center justify-center  gap-5 dropdown">
                                                {{-- <a href=""><li class="hover:!bg-gray-300"><span class="text-16 font-solaimans"> {{ __('messages.edit') }}</span></li></a> --}}
                                                <a href='{{ url('delete_share_file_own', $share->shared_id) }}'>

                                                    <li class="hover:!bg-gray-300" style="list-style: none;"><span
                                                       
                                                            class="text-16 font-solaimans" style="display: flex;gap: 5px;padding: 4px 20px;border: 1px solid red;border-radius: 4px;"> <i class="fa-solid fa-trash mt-1"></i>{{ __('messages.delete') }}</span>
                                                    </li>
                                                </a>
                                        </div>
                                    @endif
