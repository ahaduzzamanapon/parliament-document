    ...
    <div class=" bg-white rounded-md px-4 py-6 shadow">
                ...
                <div class="w-full md:w-[35%] lg:w-[75%] relative flex items-center justify-end">
                  ...
                  <div class="w-full md:w-[35%] lg:w-[30%]">
                    <a onclick="chckprmitionh_folder3()" class="bg-[#F8B200] mt-2  md:mt-0 w-full  p-2  rounded-md font-solaimans text-white">
                        ...
                        <span> ডিপার্ট্মেন্ট যোগ করুন </span>
                    </a>
                  </div>
            </div>
        </div>
            ...
            <table class="table">
              ...
              <thead class="bg-[#E9FFE4] w-full">
                <tr>
                    ...
                    <th ><h2 class="text-[#007A43] text-18 font-solaimans font-semibold ">ডিপার্ট্মেন্ট নাম </h2></th>
                ...
                <td>
                    ...
                    <td>
                      <div class="flex items-center  gap-5">
                          <a onclick="updateDepartment('{{ $dept->id}}', '{{ $dept->name}}', '{{ $dept->status}}')"  class="text-red-500 " href="#"><i class="fa fa-light fa-pencil" style="font-size: 18px;"></i></a>
                          <a onclick="return confirm('Are you sure to delete this item?')" class="text-red-500" href="{{ route('deleteDepartment', $dept->id) }}"><i class="fa fa-light fa-trash-can" style="font-size: 18px;"></i></a>
                      </div>
        ...
        <dialog id="offic_dept_addId" class="modal modal-bottom sm:modal-middle">
            <div class="modal-box max-w-xs">
                ...
                নতুন ডিপার্ট্মেন্ট তৈরি করুন
                </h3>
                <div class="my-2">
                    <form action="{{ route('storeOffDeptName') }}" method="POST">
                        @csrf
        ...
        <!-- Open the modal using ID.showModal() method -->
        <dialog id="department_update_modal" class="modal modal-bottom sm:modal-middle">
            <div class="modal-box max-w-xs">
                <h3 class="font-bold text-lg font-solaimans">
                অফিসিয়াল ডিপার্ট্মেন্ট সংশোধন
                </h3>
                <div class="my-2">
                    <form action="{{ route('updateDepartment') }}" method="POST">
                        ...
                        <div class="text-end mt-3 space-x-2">
                            <button onclick="modalClose(event, 'department_update_modal')"
                                class="text-[#007A43] font-solaimans hover:bg-gray-300 rounded-lg px-1 py-1"
                                type="button">{{ __('messages.cancel') }}</button>
                            <button
                                class="text-[#007A43] font-solaimans hover:bg-gray-300 rounded-lg px-1 py-1 transition-shadow"
...
@section('scripts')
    <script>
        function updateDepartment(id, name, status) {
            var ability = {{ Setting::upload_file() }}
                ...
                department_update_modal.showModal();
                $('#ot_name').val(name)
