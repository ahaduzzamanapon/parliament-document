...
<div class="w-full px-5 lg:ps-8 pb-4 bg-[#F2F8FF]"> 
     <h1 class="text-24 font-solaimans font-medium leading-[1.5em] pt-5">{{ __('messages.Welcome back') }}, <span class="text-[#004407] font-solaimans italic font-light leading-none">{{ __('messages.Admin') }}!</span> </h1>
    <p class="font-solaimans text-15 text-black leading-4">{{ __('messages.Dashboard') }}</h1>
    </p>
</div>
@if(Auth::user()->emp_type == 'superadmin')

<div class="w-full px-5 lg:ps-8 lg:pe-10 flex flex-col lg:flex-row items-center gap-4">
    <div class="w-full h-auto lg:w-[33%] xl:h-[8.375em] border border-[#E3F0FF] bg-white shadow-md rounded-md">
        <div class="flex w-full h-full justify-between items-center lg:place-items-start lg:gap-[1.875em] xl:gap-[3.75em] px-5 pt-5 pb-2">
            <div class="flex flex-col gap-2">
                <div class="flex flex-col items-start justify-between gap-3">
                    <h2 class="font-solaimans text-16 lg:text-18 xl:text-20 font-normal leading-5">{{ __('messages.Total User') }}</h2>
                    <h1 class="font-solaimans text-20 lg:text-24 xl:text-28 font-semibold text-[#007A43] leading-8">
                        {{ __('messages.Current_Language') == 'en' ? $totalUser : Converter::en2bn($totalUser) }}
            ...
            <div class="w-[21%] p-2 border border-[#DDECFF] bg-[#E5F1FF] rounded-md flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="38" height="39" viewBox="0 0 38 39" fill="none">
                    <path d="M19 19.5C24.2473 19.5 28.5 15.1354 28.5 9.75C28.5 4.36465 24.2473 0 19 0C13.7527 0 9.5 4.36465 9.5 9.75C9.5 15.1354 13.7527 19.5 19 19.5ZM26.1102 21.9832L22.5625 36.5625L20.1875 26.2031L22.5625 21.9375H15.4375L17.8125 26.2031L15.4375 36.5625L11.8898 21.9832C6.59805 22.2422 2.375 26.683 2.375 32.175V35.3438C2.375 37.3623 3.9707 39 5.9375 39H32.0625C34.0293 39 35.625 37.3623 35.625 35.3438V32.175C35.625 26.683 31.402 22.2422 26.1102 21.9832Z" fill="#007A43" />
                </svg>
            </div>
        </div>
    </div>
    <div class="w-full h-auto lg:w-[33%] xl:h-[8.375em]  border border-[#E3F0FF] bg-white shadow-md rounded-md">
        <div class="flex w-full h-full justify-between items-center lg:place-items-start lg:gap-[1.875em] xl:gap-[3.75em] px-5 pt-5 pb-2">
            <div class="flex flex-col gap-2">
                <div class="flex flex-col items-start justify-between gap-3">
                    <h2 class="font-solaimans text-16 lg:text-18 xl:text-20 font-normal leading-5">{{ __('messages.Total Folder') }}</h2>
                    <h1 class="font-solaimans text-20 lg:text-24 xl:text-28 font-semibold text-[#007A43] leading-8 mt-5">
                        {{ __('messages.Current_Language') == 'en' ? $totalFolders : Converter::en2bn($totalFolders) }}
                    </h1>
                </div>
        ...
        <div class="flex w-full h-full justify-between items-center lg:place-items-start lg:gap-[1.875em] xl:gap-[3.75em] px-5 pt-5 pb-2">
            <div class="flex flex-col gap-2">
                <div class="flex flex-col items-start justify-between gap-3">
                    ...
                    <h1 class="font-solaimans text-20 lg:text-24 xl:text-28 font-semibold text-[#007A43] leading-8">
                        {{ __('messages.Current_Language') == 'en' ? $totalFiles : Converter::en2bn($totalFiles) }}
                    </h1>
                </div>
    ...
    <div class="py-2 w-full border border-[#E3F0FF] bg-white shadow-md ps-[1.125em]">
        ...
        <div class="mt-3 pe-6">
                ...
                <button @if(Auth::user()->emp_type == 'vip_official') onclick="vipOfficial_upload()" @elseif(Auth::user()->emp_type == 'sochebaloy_official') onclick="chckprmitionh_folder2()" @else onclick="fileUpNotPermns()" @endif class="bg-[#F8B200] mt-2 md:mt-0 w-full md:w-[20%] py-2 rounded-md openfile" id="vipOfficial_upload">
                    <div class="wull h-full flex px-2  justify-center items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="19" viewBox="0 0 18 19" fill="none">
                            <path d="M15.75 8.67294V9.79794H9V16.5479H7.875V9.79794H1.125V8.67294H7.875V1.92294H9V8.67294H15.75Z" fill="white" />
                        </svg>
            ...
            <div class="lg:flex justify-between items-center">
                ...
                <div class="w-full">
                    @foreach ($officalRecentFolders as $item2)
                <div class="w-full my-4 flex flex-wrap items-center gap-4">
                        <div class="w-[100%] bg-[#f6f5f5] px-2 py-1">
                            <h3>{{ $item2->name }}</h3>
                        </div>
                        @if ($officalRecentFolders->count() == 0)
                        <p>{{__('messages.No Data Found')}}</p>
                        @endif
                        @foreach ($item2->categories as $item)
                        <div class="w-[100%] md:w-[50%] lg:w-[23%]  xl:w-[23.5%] flex flex-wrap items-center  gap-1  xl:gap-3" >
                            <div ondblclick="go_folder('{{$item->id}}')" class="folderclick cursor-pointer  w-[95%] bg-[#EBFBEC] py-3 rounded-sm flex items-center justify-between gap-1">
                                <div class="flex items-center ps-3 gap-2 w-full">
                                    ...
                                    <h3 style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;height: 20px;width: 70%;" class="text-10 select-none lg:text-13 lx:text-16 font-solaimans leading-4">
                                        {{$item->name}}
                                    </h3>
                                    <svg data-id="{{$item->id}}" data-name="{{$item->name}}" data-type="folder" onclick="get_hiden_menu(event, this)" class="file_delete2 w-[1.125em] cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
            ...
            @else 
                ...
                <div class="lg:flex justify-between items-center">
                    ...
                    <div class="w-full my-4 flex flex-wrap items-center gap-4">
                        ...
                        <p>{{__('messages.No Data Found')}}</p>
                        @endif
                        @foreach ($officalRecentFolders as $item)
                            <div class="w-[100%] md:w-[50%] lg:w-[23%]  xl:w-[23.5%] flex flex-wrap items-center  gap-1  xl:gap-3" >
                                <div ondblclick="go_folder('{{$item->id}}')"
                                    ...
                                    <div class="flex items-center ps-3 gap-2 w-full">
                                        ...
                                        <svg data-id="{{$item->id}}" data-name="{{$item->name}}" data-type="folder"
                                            onclick="get_hiden_menu(event, this)" class="file_delete2 w-[1.125em] cursor-pointer"
        ...
        <div class="mt-3 pe-6">
            ...
            <div class="lg:flex justify-between items-center md:flex ">
                <h3 class="font-solaimans text-20 leading-5 bg-[#b1afaf] px-2 py-1"> {{__('messages.vip Uploaded Folders')}}</h3>
            ...
            <div class="lg:flex justify-between items-center">
                <div class="w-full my-4 flex flex-wrap items-center gap-4">
                    @if ($vipRecentFolders->count() == 0)
                    <p> {{__('messages.No Data Found')}}</p>
                    @endif
                    <div class="w-full my-4 flex flex-wrap items-center gap-4" id="vip_fetch_folder_data">
...
<div class=" w-full py-3 px-5 lg:ps-8 lg:pe-10">
    ...
    {{-- <section class="container lg:flex w-full lg:gap-3  py-3">
        ...
        <!-- <div
            class="w-full mt-5 lg:mt-0 lg:w-[45%] xl:w-[40%] py-2 border border-[#E3F0FF] bg-white shadow-md rounded-md">
                    ...
                    <tbody class="py-2">
                        <tr class="shadow-md">
                            ...
                            <td><span
                                    ...
                                    User</span></td>
                            <td
                                class="font-solaimans text-10 lg:text-12 xl:text-14  font-semibold text-[#333333] leading-3">
                                30 March 2023
                                <p class="font-solaimans  text-10 lg:text-12 xl:text-14   text-[#333333] leading-3">8:15
                                    AM</p>
                            </td>
    ...
    @if(checkRole(Auth::user()->id) != 'gen_user')
    <section class="w-full h-auto pb-6 mt-5">
        <div class="border border-[#E3F0FF] bg-white shadow rounded-md lg:flex">
            <div class="w-full lg:w-[27%] py-5 ps-5 pe-5">
                <div class="flex flex-col gap-1 lg:gap-4">
                    <h1 class="text-20 lg:text-24 xl:text-32 font-solaimans text-[#000000] leading-10">{{__('messages.Storage Space Managment')}}</h1>
                    </h1>
                ...
                <div class="w-full h-[2.188em] mt-8 mb-12 bg-[#EFEFEF] rounded-md">
                    <div class="w-full  h-full flex">
                        ...
                        <div class="w-[{{$extension['percentage']}}%] h-full bg-[{{ $extension['color'] }}]" title="{{$key}}-{{$extension['size']}}">
                        </div>
                        @endforeach
                    </div>
                </div>
    ...
    <div id="open-file-section" class="absolute w-[12.5em] h-auto bg-white rounded-sm shadow-md hidden">
        <div class="w-full space-y-2 py-2 ">
            ...
            <div class="flex items-center justify-start gap-1 px-2 hover:bg-gray-300 py-1">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M11.25 3.375H14.625V4.5H13.5V14.625L12.375 15.75H4.5L3.375 14.625V4.5H2.25V3.375H5.625V2.25C5.625 1.95163 5.74353 1.66548 5.9545 1.4545C6.16548 1.24353 6.45163 1.125 6.75 1.125H10.125C10.4234 1.125 10.7095 1.24353 10.9205 1.4545C11.1315 1.66548 11.25 1.95163 11.25 2.25V3.375ZM10.125 2.25H6.75V3.375H10.125V2.25ZM4.5 14.625H12.375V4.5H4.5V14.625ZM6.75 5.625H5.625V13.5H6.75V5.625ZM7.875 5.625H9V13.5H7.875V5.625ZM10.125 5.625H11.25V13.5H10.125V5.625Z" fill="#333333" />
                </svg>
                <a id="Delete" class="text-14 font-solaimans font-semibold leading-3" onclick='confirmDelete(event); dropdownFileHide()' data-type=1>{{ __('messages.delete') }}</a>
                <!-- {{--data_type 1 for folder 2 for files--}} -->
1.1315 1.66548 11.25 1.95163 11.25 2.25V3.375ZM10.125 2.25H6.75V3.375H10.125V2.25ZM4.5 14.625H12.375V4.5H4.5V14.625ZM6.75 5.625H5.625V13.5H6.75V5.625ZM7.875 5.625H9V13.5H7.875V5.625ZM10.125 5.625H11.25V13.5H10.125V5.625Z"
                        fill="#333333" />
                </svg>
                <a id="Delete" class="text-14 font-solaimans font-semibold leading-3" onclick='confirmDelete(event); dropdownFileHide()'
                    data-type=1>{{ __('messages.delete') }}</a> 
                    <!-- {{--data_type 1 for folder 2 for files--}} -->
            </div>
            @endif
        </div>
